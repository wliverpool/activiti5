        ...
        $(function() {
           $('.delete-btn').click(function() {
              var reason = prompt("请输入删除原因：");
               $.post(ctx + '/deleteProcessinstance/' + $(this).data('pid'), {
                   deleteReason: reason
               }, function(resp) {
                   if (resp) {
                       location.reload();
...
<table width="100%" class="table table-bordered table-hover table-condensed">
    ...
    <c:forEach items="${page.result }" var="pi">
        <tr>
            <td>
                <a href="${ctx}/processTraceView/${pi.id}" target="_blank">${pi.id}</a>
            </td>
            ...
            <td>
                ...
                <c:if test="${pi.suspended}">
                    <a class="btn btn-small state-btn" href="${ctx }/processinstanceManager/active/${pi.id}"><i class="icon-ok"></i>激活</a>
                </c:if>
                <c:if test="${!pi.suspended}">
                    <a class="btn btn-small state-btn" href="${ctx }/processinstanceManager/suspend/${pi.id}"><i class="icon-lock"></i>挂起</a>
                </c:if>
            </td>
            <td>
