...
<head>
	...
	<script type="text/javascript">
		var taskId = '${hasFormKey ? task.id : taskFormData.task.id}';
		var processInstanceId = '${hasFormKey ? task.processInstanceId : taskFormData.task.processInstanceId}';
	</script>
	<script type="text/javascript" src="${ctx }/js/events.js"></script>
	<script type="text/javascript" src="${ctx }/js/task-form.js"></script>
</head>
<body>
	...
	<p class="task-infos text-left">
		...
		<c:if test="${not empty parentTask}">
			...
			<c:if test="${empty parentTask.endTime}">
				<div>
					该任务属于：<a href="${ctx}/doTask/${parentTask.id}">${parentTask.name}</a>
				</div>
			</c:if>
			...
			<c:if test="${not empty parentTask.endTime}">
				<div>
					该任务属于：<a href="${ctx}/taskArchived/${parentTask.id}">${parentTask.name}</a>
				</div>
			</c:if>
		</c:if>
	</p>
		...
		<ul class="unstyled">
			...
			<li>
				...
				<c:if test="${not empty subTask.endTime}">
					<a href="${ctx}/taskArchived/${subTask.id}" class="task-name">
	...
	<div>
		...
		<ul class="unstyled">
			...
			<li>
				...
				<%-- 文件类型的文件 --%>
				<c:if test="${attachment.type != 'url'}">
					<a href="${ctx}/attachmentDownload/${attachment.id}" target="_blank" data-toggle="tooltip" title="${attachment.description}" data-placement="right" class="attachment-item"><i class="icon-file"></i>${attachment.name}</a>
				</c:if>
	...
	<hr/>
	<form action="${ctx }/completeTask/${hasFormKey ? task.id : taskFormData.task.id}" class="form-horizontal" method="post">
				...
				<c:if test="${not empty task.delegationState}">
					<c:if test="${task.delegationState == 'PENDING' }">
						<c:if test="${task.assignee == sessionScope.user.id}">
							<a class="btn" href="${ctx }/taskDelegateComplete/${task.id}"><i class="icon-ok"></i>完成</a>
						</c:if>
					</c:if>
				</c:if>
				<a href="javascript:history.back();" class="btn"><i class="icon-backward"></i>返回列表</a>
				<c:if test="${!processInstance.suspended}">
					<c:if test="${empty task.delegationState||task.delegationState=='RESOLVED'}">
							...
							<c:forEach items="${identityLinksForTask}" var="link">
								...
								<c:choose>
									<c:when test="${link.type == 'candidate'}">
										<c:if test="${not empty link.userId&&link.userId == sessionScope.user.id}">
											<a class="btn" href="${ctx }/claimTask/${task.id}?nextDo=handle"><i class="icon-ok"></i>签收</a>
										</c:if>
										<c:if test="${empty link.userId && not empty link.groupId}">
											<c:forEach items="${sessionScope.groups}" var="userGroup">
												<c:if test="${userGroup.id == link.groupId}">
													<a class="btn" href="${ctx }/claimTask/${task.id}?nextDo=handle"><i class="icon-ok"></i>签收</a>
												</c:if>
											</c:forEach>
										</c:if>
										<c:set var="type" value="候选${not empty link.userId ? '人' : '组'}" />
									</c:when>
								</c:choose>
							</c:forEach>
						</c:if>
	...
	<div id="addSubtaskModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="addSubtaskModalLabel" aria-hidden="true">
	  ...
	  <div class="modal-body">
	    <form id="subTaskForm" action="${ctx}/taskSubtaskAdd/${task.id}" class="form-horizontal" method="post">
	    	<div class="control-group">
	  ...
	  <div class="modal-body">
		...
		<div id="my-tab-content" class="tab-content">
		    <div id="file" class="tab-pane active">
		    	<form id="fileAttachmentForm" action="${ctx}/attachmentAddFile" class="form-horizontal" method="post" enctype="multipart/form-data">
		    ...
		    <div id="web" class="tab-pane">
		    	<form id="urlAttachmentForm" action="${ctx}/attachmentAddUrl" class="form-horizontal" method="post">
		    		<input type="hidden" name="taskId" value="${task.id}" />
		    		<input type="hidden" name="processInstanceId" value="${task.processInstanceId}" />
					<div class="control-group">
