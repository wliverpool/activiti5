...
function participants() {
	...
	$('#sendAddParticipantsRequest').click(function() {
		...
		$.post(ctx + '/taskParticipantAdd/' + taskId, {
			userId: userIds,
			type: types
		}, function(resp) {
			if(resp == 'success') {
	...
	$('.link-delete').click(function() {
		var ele = this;
		$.post(ctx + '/taskParticipantDelete/' + taskId, {
...
function candidate() {
	...
	$('#sendAddCandidateRequest').click(function() {
		...
		$.post(ctx + '/taskCandidateAdd/' + taskId, {
...
function subTask() {
	$('.subtask-delete').click(function() {
		...
		location.href = ctx + '/taskSubTaskDelete/' + $(this).data('taskid');
	});
}
...
function attachment() {
	...
	$('.attachment-delete').click(function() {
		var ele = this;
		if(confirm('确定删除附件？')) {
			$.get(ctx + '/attachmentDelete/' + $(this).data('id'), function(resp) {
				if(resp == 'true') {
					$(ele).parent().remove();
				} else {
					alert('删除失败！');
