...
<head>
    ...
    <script type="text/javascript" src="${ctx }/js/trace-process.js"></script>
</head>
<body>${activeActivities}
    <%-- 先读取图片再通过Javascript定位 --%>
    <div>
        <img id="processDiagram" src="${ctx }/readResource?pdid=${historicProcessInstance.processDefinitionId}&resourceName=${processDefinition.diagramResourceName}" />
    </div>

    <%-- 通过引擎自动生成图片并用红色边框标注
    <div>
        <img id="processDiagramAuto" src="${ctx }/traceDataAuto/${historicProcessInstance.processInstanceId}" />
    </div>--%>
    <hr>
    <fieldset>
