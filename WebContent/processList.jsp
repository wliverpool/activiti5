...
$(function () {
    ...
    $('.change-state').click(function () {
        ...
        $('#changeStateForm').attr('action', ctx + '/processDefinitManage/' + state);
    });
	...
	//后台查询流程定义id是否有已经设置的有权限使用该流程的候选参与人和组
    $.getJSON(ctx + '/processStartableRead/' + pid, function(result) {
    		//如果有已经设住的数据,在显示的候选人以及组的选项中已经设置过的人或者组为选中状态
    ...
    $('#addStartableAttr').click(function () {
        ...
        // 发送请求
        $.post(ctx + "/processStartableSet/" + processDefinitionId, {
            users: users,
            groups: groups
        }, function (resp) {
            if (resp == 'true') {
...
<c:if test="${not empty error}">
    <div id="message" class="alert alert-success">${error}</div>
    <!-- 自动隐藏提示信息 -->
    <script type="text/javascript">
        setTimeout(function () {
            $('#message').hide('slow');
	...
	<c:forEach items="${page.result}" var="pd" varStatus="row">
		<tr index="${row.index}">
            ...
            <td><a target="_blank" title="资源名称：${pd.resourceName }"
                   href='${ctx }/readResource?pdid=${pd.id }&resourceName=${pd.resourceName }'>查看</a></td>
            <td><a target="_blank" title="资源名称：${pd.diagramResourceName }"
                   href='${ctx }/readResource?pdid=${pd.id }&resourceName=${pd.diagramResourceName }'>查看</a>
            </td>
            ...
            <td>
                <c:if test="${not empty linksMap[pd.id]['user'] || not empty linksMap[pd.id]['group']}">
                    <a href="#" class="toggle-startable" data-index="${row.index}">
                        	人[${fn:length(linksMap[pd.id]['user'])}],组[${fn:length(linksMap[pd.id]['group'])}]
                    </a>
                </c:if>
            </td>
            <td>
                <div class="btn-group">
                    ...
                    <ul class="dropdown-menu" style="min-width: 100px;margin-left: -50px;">
                        <li><a href='${ctx }/deleteDeploy?deploymentId=${pd.deploymentId }'><i class="icon-trash"></i>删除</a></li>
                        <li><a href="${ctx }/toStartDefinition/${pd.id}"><i class="icon-ok"></i>启动</a></li>
                        <li><a class="set-startable" data-pid="${pd.id}" title="设置候选启动人|组"><i class="icon-user"></i>候选启动</a></li>
                        <c:if test="${!pd.suspended}">
                            <li><a href="#changeStateModal" class="change-state" data-state="suspend" data-pid="${pd.id}" data-toggle="modal"><i class="icon-lock"></i>挂起</a></li>
                        </c:if>
                        <c:if test="${pd.suspended}">
                            <li><a href="#changeStateModal" class="change-state" data-state="active" data-pid="${pd.id}" data-toggle="modal"><i class="icon-ok"></i>激活</a></li>
                        </c:if>
                    </ul>
                </div>
            </td>
...
<!-- 流程定义挂起 -->
<div id="changeStateModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="changeStateModalModalLabel" aria-hidden="true">
